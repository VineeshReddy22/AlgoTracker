function liveContactEmail() {
  ...
  var re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (em.value.length === 0) { errEl.innerText = ""; return; }
  if (!re.test(em.value)) errEl.innerHTML = '<span style="color:#ef4444;">&#10007; Invalid email</span>';
  else errEl.innerHTML = '<span style="color:#22c55e;">&#10003; Looks good</span>';
}
function liveContactPhone() {
  ...
  if (ph.value.length === 0) { errEl.innerText = ""; return; }
  if (ph.value.length < 10) errEl.innerText = ph.value.length + "/10 digits";
  else errEl.innerHTML = '<span style="color:#22c55e;">&#10003; Valid</span>';
}
function submitContactForm(e) {
  ...
  function clr(id) { document.getElementById(id).innerText = ""; }
  ...
  if (topics.length === 0) { document.getElementById("cTopicErr").innerText = "Select at least one topic."; ok = false; }
  else document.getElementById("cTopicErr").innerText = "";
  if (msg.value.trim() === "") err("cMessageErr", "Message cannot be empty.", msg);
  else clr("cMessageErr");
  if (!decl.checked) { document.getElementById("cDeclErr").innerText = "Please check the declaration."; ok = false; }
  else document.getElementById("cDeclErr").innerText = "";
  if (focusTarget) focusTarget.focus();
  if (ok) {
    document.getElementById("formSuccess").innerHTML = "&#10003; Your message has been sent successfully!";
